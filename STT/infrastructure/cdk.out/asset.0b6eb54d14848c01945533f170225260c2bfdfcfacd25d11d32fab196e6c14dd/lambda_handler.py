...
async def transcribe_elevenlabs(audio_base64, stream_sid, connection_id):
    ...
    print(f"11LABS_CONNECT | audio_size={audio_size_kb:.1f}KB")
    ...
    try:
        async with websockets.connect(ws_url, additional_headers=headers) as ws:
            ...
            print(f"11LABS_SESSION | {session_msg[:200]}")
            
            # Send complete audio
            await ws.send(json.dumps({"audio_base_64": audio_base64}))
            print(f"11LABS_SENT | audio sent")
            
            # Send EOS and commit
            await ws.send(json.dumps({"type": "eos"}))
            await ws.send(json.dumps({"type": "flush"}))
            print(f"11LABS_COMMIT | eos+flush sent")
            
            # Get transcript - wait for all messages
            transcript_text = ""
            while True:
                try:
                    ...
                    elif msg_type == "eos_received":
                        print(f"11LABS_EOS_ACK")
                    
                    elif msg_type == "done":
                        latency_ms = (time.time() - send_time) * 1000
                        # Use accumulated transcript if we got one
                        if transcript_text:
                            print(f"TRANSCRIPT | latency={latency_ms:.0f}ms | text=\"{transcript_text}\"")
                            save_transcript(stream_sid, transcript_text, connection_id)
                        else:
                            print(f"11LABS_DONE | no transcript")
                        break
                    ...
                    print("11LABS_TIMEOUT | no response in 15s")
                    break
