...
Disables cache/memory operations for stateless execution.
"""
...
class NoOpCache(ICache):
    """
    No-op implementation of ICache that doesn't store or retrieve data.
    
    Useful for:
    - Stateless executions without caching
    - Testing without cache infrastructure
    - Development environments
    - Tools that don't need caching
    
    Usage:
        cache = NoOpCache()
        
        # All operations are no-ops
        await cache.set("key", "value")  # Doesn't store
        value = await cache.get("key")  # Returns None
        exists = await cache.set_if_absent("key", "value")  # Returns False
        await cache.delete("key")  # No-op
        
        # Lock is no-op context manager
        async with cache.lock("resource"):
            # No actual locking
            pass
    """
    ...
    async def get(self, key: str) -> Any:
        """
        Get value from cache (no-op implementation).
    ...
    async def set(self, key: str, value: Any, ttl_s: Optional[int] = None) -> None:
        """
        Set value in cache (no-op implementation).
    ...
    async def delete(self, key: str) -> None:
        """
        Delete value from cache (no-op implementation).
        
        Args:
            key: Cache key to delete (ignored)
        """
        pass
